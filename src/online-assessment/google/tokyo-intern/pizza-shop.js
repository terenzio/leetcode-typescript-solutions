"use strict";
const pizzaShop = (pizzas, toppings, x) => {
    const perms = [];
    const set = new Set();
...
const generatePerms = (pizza, stockToppings, addedToppings = [], perms = []) => {
    if (addedToppings.length <= 2) {
        ...
        for (let i = 0; i < stockToppings.length; i++) {
            ...
            generatePerms(pizza, nextStockToppings, [...addedToppings, stockToppings[i]], perms);
        }
    }
    return perms;
};
const getClosestNumber = (nums, x) => {
    if (nums.length <= 1) {
        return x;
    }
